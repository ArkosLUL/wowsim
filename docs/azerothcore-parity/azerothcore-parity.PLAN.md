...
- Dot ticks sit on `NextServerTick` of their nominal time while the nominal lattice keeps running, so the
  leftover carries into the next tick (`AuraEffect::Update`).
...
- `server_tick_test.go`: `NextServerTick`, the per-iteration phase, swings on the tick (2.55 s → 2.6 s), haste
  ...
  and hardcast completion on the tick, dot ticks on the lattice, a channel ending with its last tick.
...
- UI: `components/server_settings_picker.ts`, the "Server (AzerothCore)" section of the individual settings tab (not inside the raid
  ...
  - Inputs: raid difficulty; a Global/Health/Armor/Damage × Boss/Other grid that edits the selected difficulty's size set (a live size key
    ...
    exotic pet damage %.
  - The map update interval input comes with P3-3, and reforging's enable, percentage and stat list with P6-1.
...
tab and acraid's export. PAR-P6-1 took its percentage and stat list from P5's server settings and moved the
rule onto `UIItem`/`SimItem.server_stats`, the compacted `item_template` stat rows, so the sim now refuses
the sources the server refuses.
...
- Talent/glyph values and class masks. Percent damage spell mods (`SPELLMOD_DAMAGE`, `SPELLMOD_DOT`) multiply
  (`Player::ApplySpellMod`), as `sim/paladin`'s `spellModDamage` does.
...
1. **Death Knight** (G, done; facts in the INVESTIGATION):
   - Disease add-tick haste with Epidemic.
   - Virulence, Nerves of Cold Steel and Rage of Rivendare values (the sim gives Rage of Rivendare 1
     expertise a point, the server 2).
   - Blood Plague/Frost Fever tick crits (both are melee damage class on the server).
   - Ghoul: 70/30 scaling, owner haste and ArP, avoidance.
   - The DK's summons carry 67561, not 61017: no melee hit, and only risen ghouls, pet or guardian,
     take ArP (INVESTIGATION, Pets). Core needs a marker to tell them apart, which this item may add to
     `sim/core/pet.go`.
   - Gargoyle: 75% AP→SP.
...
2. **Hunter (recorded)** (G, done; facts in the INVESTIGATION):
...
3. **Rogue** (H, done; facts in the INVESTIGATION):
...
4. **Warrior** (H, done; facts in the INVESTIGATION):
...
5. **Retribution Paladin (recorded)** (H, done; facts in the INVESTIGATION):
   - CS/DS apply seal stacks; seal DoT crit.
   - Glyph of Reckoning (`proto/paladin.proto:122`, spell 67485) has a server script (mod-spell-tweaks) but no
     client data, so no player can have it and the sim registers nothing for it. Model it once the client has it.
...
11. **Tanks** (Prot Warrior, Prot Paladin (recorded), Bear, DK tank):
    ...
    - The DK's rune weapon mirrors the DK's white swings and every spellbook spell he lands
      (`spell_dk_dancing_rune_weapon`). The sim mirrors Death Coil off the dummy cast's `OnCastComplete`, so
...
- Pets still on agility crit (warlock pets, the infernal, treants, spirit wolves) take the server's 5% plus
  crit auras (INVESTIGATION, Pets).
- Guardians take raid buffs since P7-0d: spot-check the class's (Water Elemental, fire and earth elementals,
  treants, Shadowfiend).
...
- Warlock pet hit from Classic tests (`sim/warlock/pet.go:313-340`).
...
- Hunter leftovers from wave G (INVESTIGATION, Hunter): pet focus arrives 24 every 4 s, and the pet AI
  pools 50 focus for Furious Howl and Savage Rend where the server's `PetAI` doesn't; Kill Shot's weapon part
  isn't normalized on the server, and its extra 0.4 RAP leaves out Hunter's Mark; `spell_bonus_data` AP
  coefficients read `RangedAttackPower`, creature-type AP included, which `Unit::SpellDamageBonusDone` leaves
  out; Thori'dal's own 15% ranged haste (44972) is unmodeled; the hunter suites' `FerocityTalents` is a
  16-point build, so no golden uses Beast Mastery's 22; in the BM capture the pet idled through 2 of its 3
  Bestial Wraths, cause unknown; Auto Shot and Steady Shot crit run about 2.6 points over the sim's (1.7σ pooled,
  SV + wasp +2.49%) with no missing term found, which a live `.simval` crit probe would settle.
- The DK APLs aren't retuned for the disease snapshot: diseases applied under Bloodlust keep that amplitude
  all fight, which pays off for Blood and Frost/Unholy.
- Rogue leftovers from wave H (INVESTIGATION, Rogue): Killing Spree's swings register as 51690 with tags 1 and 2,
  not 57841/57842, so the serverdata lookup misses them; Instant and Wound Poison's PPM model needs a live proc
  capture on the poison enchant ids; Master Poisoner's per-rogue count is exact only while one target carries the
  poison.
- Warrior leftover from wave H: Shattering Throw's `hasGlyph` still switches stance and throws from Berserker
  Stance for a hand-picked Glyph of Shattering Throw (206953), which the server doesn't have.
- Paladin leftovers from wave H: `canJudgement` (`judgement.go`, needs an active seal) isn't wired into
...
- **Core files:**
  - Class items never edit `sim/core/*`. A core fix a class item needs becomes a follow-up P7-0c item.
    Except PAR-P7-MAG (I): `delayed_periodic.go`, by the user's call, and `apl_values_spell.go`'s cast time.
...
| PAR-P3-5 (E, done) | DoT refresh rule; integer-ms ticks; `TickHaste` modes; `TicksCanCrit`. Put periodic ticks on `sim.NextServerTick` of their nominal time, keeping the nominal lattice so the leftover carries over (`AuraEffect::Update`), then drop P3-3's channel exemption (`Spell.keepChannelExpiryExact`, `Aura.exactExpiry`) | `sim/core/{dot,periodic_action,spell_outcome}.go`, `cast.go` and `aura.go` (dropping the exemption) + tests | nearly all | P3-1; P3-3 (soft) |
| PAR-P6-1 (E, done) | `UIItem`/`SimItem.server_stats`; reforge % and stat list from `Character.Server().Reforging` (the server floors `float32(value) × pct/100`; disabled drops reforges); generate the reforge limits `server_settings.go` hand-copies today; the "fewer than 10 stats" rule; item maps only through `AddToDatabase`/`Lookup*`; keep `core.CanReforge`'s signature (BIS-rules' `pool.go` calls it) or request the change; an in-game reforge e2e test; reforge inputs in the Server section. `pool.go`'s reforge options then follow the request's `ServerSettings`, and its template-only rule reads `server_stats` (BIS-rules' contract request): today a rating only an equip spell gives, and equal melee and spell stat types, pass as sources the server refuses. **Done in wave E**, with `core.CanReforge`/`ReforgeStats` now taking the item and the run's `Reforging` (contract change), `gen_server_defaults` reading the limits out of `item_reforge.h`, and `e2e/reforge_test.go` buying a reforge from the live reforger NPC over the gossip protocol (which needed a `SendRaw` in `harness.patch`) | `sim/optimizer/pool.go` (reforge options), `ui/core/components/server_settings_picker.ts`, `tools/acore/gen_server_defaults/`, `proto/ui.proto`, `proto/common.proto` (SimItem), `sim/core/{database,database_load,reforging,bulksim}.go`, `reforging.ts`, `gear_picker.tsx`, `tools/database/azerothcore/roster.go` | none | AC-1, P5-1, BIS-contract |
| PAR-P7-0a (F, done) | Audit buffs, debuffs, consumes and racials against the spelldump; the AoE cap. Thorns (53307) isn't binary on the server: drop `SpellFlagBinary` in `buffs.go` and the shared allowlist entry. **As built:** four Classic values were wrong (Blessing of Wisdom 92 against Mana Spring's 91, so they no longer share a number; Demoralizing Screech 574; Curse of Weakness at "regular" was paying the improved rank), a talent's percentage on an aura amount truncates (`addPct`), and Booming Voice is +25% a rank on Demoralizing Shout too. The AoE cap stays: the server caps a player's own launch damage at ten targets, but not a pet's, totem's or a tick's, which class items still owe. Consumes and racials needed no change, and no new spell-id constant, so the generated serverdata didn't move | `sim/core/{buffs,debuffs,consumes,racials}.go`, `target.go` (`updateAOECapMultiplier`) | all 37 | P3-4, P5-23 |
| PAR-P7-0b (F, done) | Pet core: owner hit and expertise floored and refreshed every 3 s; haste and ArP inheritance (with spell tweaks' hunter pet and ghoul ArP, pets take the owner's ArP rating and percent-ArP auras, filtered by the pet spell's class mask; Blood Gorged lives on the DK's spells, so the ghoul's white swings need it explicitly); pet avoidance. **As built:** 20 pet suites moved. The DK's summons carry a scaling aura that grants no melee hit, which core can't tell from a bloodworm's without a marker, so that goes to PAR-P7-DK. The hunter gain is inflated by the sim's x1.15 ranged hack, which PAR-P7-HUN removes | `sim/core/{pet,avoid_dr,unit}.go` | 20 pet suites | P3-3, P5-1 |
...
| PAR-P7-0d (H2, done) | Core follow-ups from wave G. A hardcast queued behind the GCD lands a tick earlier on the server: hasted Steady Shots chained every 1.5 s there and 1.6 s in the sim, most of the SV + wasp recorded run's +3.00% (INVESTIGATION, Hunter); find the server's rule before changing `cast.go`. Auto Shot and Steady Shot crit ran about 3 points over the sim's in all three hunter runs (about 2σ) while Arcane Shot matched: find the missing term. Guardians take party auras and Bloodlust unless a scaling aura makes them immune (bloodworms get Bloodlust, Windfury, Improved Icy Talons and AP% auras); the sim gives guardians none. `BloodlustAura` still hastes the permanent DK ghoul directly, which 51996's `MELEE_SLOW` immunity blocks. Pet scaling and inherited haste resnapshot every 2 s on the server (hunter pet, permanent ghoul), where the sim is continuous. P7-0c's swing leftovers: a haste change during Slam rescales its pause, where the server rescales the frozen timer; a `CancelAutoSwing` from the main hand's last APL check still lets that swing go; channels don't hold swings. Black Bow of the Betrayer (32336) panics on a non-hunter (`*dps.DpsWarrior is not hunter.HunterAgent`), so the optimizer drops it for every warrior. A `core.RaidBenchmark` variant taking iteration counts, which stops writing into the shared `AverageDefaultSimTestOptions` and serves the five benches still on one iteration (Feral, Feral Tank, Enhancement, Fury, Protection Warrior) and the five copies of `benchmarkIterations`. Rune of Razorice's Frost Vulnerability is keyed by label on the target, so two Razorice DKs share one stack count, where the server keeps one per caster. **As built:** 26 suites moved, most from guardians taking raid buffs (area targeting doesn't tell pets from guardians): Blood Tank +8.8%, Blood and Frost Mage +6%. The queued-cast premise was wrong: the server truncates hasted GCDs and cast times to whole ms (`Spell::TriggerGlobalCooldown`, `Unit::ModSpellCastTime`), and the sim's float crossed a tick; the spell queue isn't involved. `Unit.HasteCarrier` and `Pet.OwnerHasteSource` replace the DK and hunter haste plumbing: a carrier takes Bloodlust only through its owner, a pet resnapshots owner speed every 2 s, a guardian once at the summon. `Spell.HoldMeleeUntil` holds melee through a channel (Army of the Dead; Bladestorm isn't a server channel). Every bench uses `core.RaidBenchmarkIterations`. The ranged crit gap stays open: no missing term found | `sim/core/{cast,attack,pet,buffs,debuffs,test_utils}.go` + tests, `sim/hunter/items.go` (Black Bow), the `BenchmarkSimulate` test files | all | P7-0c, P7-DK, P7-HUN |
| PAR-P7-0e (H3, done) | Core follow-ups from wave H. `rollYellow` never reads `SpellFlagAlwaysHit`, so an always-hit spell still rolls the sim's table, and `OutcomeMeleeSpecialCritOnly` (Slam's hit, the secondary judgements) still rolls a partial block, which `Unit::isSpellBlocked` skips for ALWAYS_HIT and NO_ACTIVE_DEFENSE spells. `Unit::CastDelayedSpellWithPeriodicAmount` applies a periodic after `EventProcessor::CalculateQueueTime(400)`, the next 400 ms step of the caster's event clock (~200 ms on average live), where Deep Wounds waits a flat 400 ms. The same path serves Unholy Blight, Piercing Shots, Righteous Vengeance, Ignite, Languish and two Shaman procs: add a core helper with a per-iteration phase and move Deep Wounds and the first three onto it, whose class items have landed; wave I's items move the rest. **As built:** `rollYellow` skips the whole table for ALWAYS_HIT, and `OutcomeMeleeSpecialCritOnly` skips the partial block for it and NO_ACTIVE_DEFENSE; that moved only Slam from the front (+0.005%). `core.DelayedPeriodicApplier` (`sim/core/delayed_periodic.go`) derives each caster's phase from the iteration's seed, so it consumes no roll. The refresh lands just ahead of a swing or tick due on its server tick, as the caster's events run first in `Player::Update` (the cross-review's fix). With it, 6 suites moved, all from the delay: Arms -2.6%, Protection Warrior -0.6%, Fury +0.4%, the rest under 0.1%. Left open: server refreshes land a mean 189 ms after the crit, the sim's 250 ms (INVESTIGATION, **DoTs and periodic ticks**), which Ignite inherits | `sim/core/spell_outcome.go`, the new helper + tests, the four proc sites (`sim/warrior/deep_wounds.go`, `sim/deathknight/talents_unholy.go`, `sim/hunter/talents.go`, `sim/paladin/talents.go`) | warrior, Ret, Unholy and hunter suites, and tanks hitting the boss's front | P7-WAR |
| PAR-P7-RET-RR (H3, done) | Recapture the Ret recorded run: wave H's ran with the bot's Protection talents under the Ret ones, since `.reset talents` lacked the name (fixed in mod-sim-validation `39e086e`). Compare it through `tools/simval rrsim` and `chronicle` against the ±2% bar. The capture casts a fixed spell cycle (its setup's `rotation:` line), so make rrsim replay that cycle or judge per ability, and explain what's left. **As built:** the APL engine can't replay a fixed cycle, so DPS compares rotations (-26.5%) and the measurement is per ability: `rrsim -v` and `chronicle -v` print per-outcome counts and averages with errors. rrsim now copies the bot (the aura from `started:`, no Avenging Wrath, swinging from the front, the recorded glyphs, a missing enchant's live-DBC stats). On the 600 s recapture `ret_Svrleadjxcky_1790084304`, every ability matches within noise, +0.30% weighted by the server's outcome counts; Righteous Vengeance only with PAR-P7-0e's delay on the merged tree (per 30% of the crit damage fed to it: server 1.40, sim 1.39). Five paladin bugs fixed: Two-Handed Weapon Specialization on holy damage, percent spell mods added instead of multiplied, Divine Storm not normalized, Seal of Vengeance's per-stack truncation, Sanctified and Swift Retribution without the paladin's own aura; Ret -1.5% | the Ret capture in `sim/core/testdata/chronicle/`, `tools/simval/rrsim*`, mod-sim-validation `e2e/recorded_run_test.go`, `sim/paladin/**` for a sim bug it finds, except Righteous Vengeance's proc (PAR-P7-0e) | Ret and Prot Paladin, only for a sim bug | P7-RET |
| PAR-PERF-2 (G, done) | Speed-ups that change results, from PAR-PERF's profile. Build Nibelung's 10 Val'kyr pets only for a player wearing it: every Balance, Shadow, Elemental, Mage and Warlock builds them today (9.4% of Elemental's CPU, 35% of its bytes), and dropping them changes the pet list and unit indices. Make `AddPendingAction` a binary search once the pending queue's sort order is checked or enforced (2% of the raid). Read `findServerAllowance` without its lock, since allowances only register at init. Then make the throughput benchmarks measure rotations: Ret, Hunter and Elemental on their golden APLs, the raid bench with its specs' `StandardTalents`, plus multi-iteration cases so the environment build stops dominating. That starts a new throughput table. **As built:** no golden moved: the Val'kyrs are always the last pets added and draw nothing unless summoned, and `nibelung_test.go` pins the count, which no golden would notice. `AddPendingAction`'s binary search keeps first-in first-out ties; an assertion build checked it against the old scan over every test, the simval replay and every bench. Every allowance registers in an init function. The benches run their suites' default players at 1 and 100 iterations: under load, Elemental 30% and the raid 40% faster at one iteration, the others unchanged. `AddPendingAction` is still 4.5-6.4% at 100 iterations | `sim/common/wotlk/{nibelung,other_effects}.go` (Nibelung's wiring), `sim/core/{sim,spell}.go` (`AddPendingAction`, `findServerAllowance`), the `BenchmarkSimulate` test files | the caster suites | PAR-PERF |
| PAR-P7-0f (I3, done) | Two core fixes wave I2 found. **Might counted twice in the stats breakdown:** `applyAllEffects` (`sim/core/character.go`) applies the attack power multiplier twice to Blessing of Might and Battle Shout in its phase snapshots, so the character stats tooltip's Buffs part reads about 75 AP high (Ranged AP for hunters; 250 Spell Dmg for Enhancement and Retribution) and the warrior's and bear's late part about 76 low. FinalStats is right. Then turn `tools/uitest/tests/gear/character_stats.spec.ts`'s breakdown fixme for the 7 specs into passing tests, dropping `MIGHT_TWICE_*` and the partial test. **Another class's set or item effect crashes:** class set bonuses and item effects cast their wearer to the class's agent (`agent.(ShamanAgent)` in `sim/shaman/items.go`, likewise per class), so a sim where another class wears an unrestricted set panics: a DK in Tidefury Raiment, a TBC mail caster set. The optimizer drops such a bonus with a warning. On the server the wearer gets the bonus, which touches only its class's spells: give it no effect, in every class, without a `recover`. P7-0d fixed Black Bow of the Betrayer alone. A test puts every unrestricted set and class item effect on each class that can wear it and runs a short sim. **As built:** FinalStats wasn't right for Enhancement and Retribution: Might's AP turned into spell power before its AP multiplier, where the server converts total AP (`Unit::SpellBaseDamageBonusDone`), so both gain 20.61 Spell Power (+0.05 to +0.30%). A class's set bonus or item effect goes through `core.ClassEffect`, which does nothing for another class's wearer; `sim/item_wearers_test.go` is the sweep. Left open: a hunter with an empty ranged slot hangs the sim (`WeaponFromRanged` gives a 0 s swing while `AutoSwingRanged` stays on) | `sim/core/character.go` (`applyAllEffects`), the item-set and item-effect code in `sim/core`, every `sim/<class>/items.go` and other file where a set or item effect casts the agent, `ui/core/components/character_stats.tsx`, `tools/uitest/tests/gear/character_stats.spec.ts`, Go tests | none expected (the goldens store only FinalStats); run all 37 and report any delta | none |
| PAR-DECL (I4) | The [effect declaration](effect-declarations.PLAN.md#par-decl-i4) on `SpellConfig`/`DotConfig` and its server check (range, coefficients, weapon percent, school, missile speed); mods; level-80 ranges in `gen_serverdata`; the undeclared list; five proof spells | `sim/core/{spell,dot}.go` and the declaration's file, `tools/acore/gen_serverdata/`, `sim/core/serverdata/`, `sim/serverdata_test.go`, `sim/hunter/pet_abilities.go`, the allowlist files and undeclared lists, the proof spells' files and the check's one-line fixes | all 37 | P3-2 |
| PAR-DECL-1 (I5) | The [class sweep](effect-declarations.PLAN.md#class-sweep-i5) for mage and warlock | `sim/{mage,warlock}/**` | their suites | PAR-DECL |
| PAR-DECL-2 (I5) | The class sweep for shaman, druid (Feral Tank included) and hunter with pets | `sim/{shaman,druid,hunter}/**` | their suites | PAR-DECL |
| PAR-DECL-3 (I5) | The class sweep for warrior, rogue, paladin and DK, tank-spec spells included | `sim/{warrior,rogue,paladin,deathknight}/**` | their suites | PAR-DECL |
...
- **Effect declarations** ([PLAN](effect-declarations.PLAN.md)): PRI declares the priest's effects and needs
  PAR-DECL. P8 keeps the `SPELLMOD_DAMAGE`/`DOT` percents and builds on the sweep's `WeaponPct`, split out
  of `DamageMultiplier`.
