...
- Paladin leftovers from wave H: `canJudgement` (`judgement.go`, needs an active seal) isn't wired into
  ...
  already do; switching moves Ret. The Ret capture ran with the bot's Protection talents under the Ret ones
  (`.reset talents` lacked the name): recapture it, and decide whether its fixed spell cycle or the sim's
  rotation is what gets compared.
