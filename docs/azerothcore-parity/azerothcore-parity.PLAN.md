...
- Talent/glyph values and class masks. Percent damage spell mods (`SPELLMOD_DAMAGE`, `SPELLMOD_DOT`) multiply
  (retail deviation 53), as `sim/paladin`'s `spellModDamage` does.
...
- Hunter leftovers from wave G (INVESTIGATION, Hunter): pet focus arrives 24 every 4 s, and the pet AI
  ...
  Bestial Wraths, cause unknown; Auto Shot and Steady Shot crit run about 3 points over the sim's (1.9σ pooled,
  SV + wasp +2.97%) with no missing term found, which a live `.simval` crit probe would settle.
...
| PAR-P8 (K) | Sweep `classic`/`wotlk-classic-bugs` in `sim/`; flip `TicksCanCrit`; multiply percent damage spell mods (retail deviation 53) in the classes that landed before H3 (DK, Hunter, Rogue, Warrior); close this plan | `sim/**` references | all | every P7 item |
