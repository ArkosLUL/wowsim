...
- Paladin leftovers from wave H: `canJudgement` (`judgement.go`, needs an active seal) isn't wired into
  Judgement of Wisdom and Light; the Ret recorded run still comes out 5.8% over the server, unexplained, and a
  second capture would tell a real gap from noise.
...
| PAR-P7-0e (unscheduled) | Core follow-ups from wave H. `rollYellow` never reads `SpellFlagAlwaysHit` from the server data, so a spell the server always hits still rolls the sim's table; Slam's damage (50783) uses `OutcomeMeleeSpecialCritOnly` meanwhile, which still rolls the separate yellow block. `Unit::CastDelayedSpellWithPeriodicAmount` applies a periodic after `EventProcessor::CalculateQueueTime(400)`, the next 400 ms step of the world clock (~200 ms on average live), where Deep Wounds waits a flat 400 ms. The same path serves Ignite, Righteous Vengeance, Piercing Shots, Unholy Blight, Languish and two Shaman procs: add a core helper with a per-iteration phase, move Deep Wounds onto it, and let the class items use it | `sim/core/spell_outcome.go`, the new helper + tests, `sim/warrior/deep_wounds.go` | warrior suites and any with an always-hit yellow spell | P7-WAR |
