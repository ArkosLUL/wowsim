...
| azerothcore-parity | Server-exact mechanics, generated constants, server settings, validation | `azerothcore-parity`; [par] | [docs/azerothcore-parity/](../azerothcore-parity/) | `tools/acore/`, `tools/simval/`, `sim/core` combat mechanics (tables, outcomes, base stats), `[ac]/modules/mod-sim-validation` (own repo) |
...
- azerothcore-parity has master's reforge, racial-traits and professions models (merged when P2 landed).
  Master's models win: P5/P6 extend them (e.g. reforge % and stat list as server settings) rather than
  rebuild them.
