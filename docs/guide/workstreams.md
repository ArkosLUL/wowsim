...
| azerothcore-parity (loop) | Server-exact mechanics, generated constants, server settings, validation | loop | [docs/azerothcore-parity/](../azerothcore-parity/) | `tools/acore/`, `tools/simval/`, `sim/core` combat mechanics (tables, outcomes, base stats), `[ac]/modules/mod-sim-validation` (own repo) |
