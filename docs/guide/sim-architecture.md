...
| `character.go`, `mana.go` | stat dependencies. `BaseStatsRace` sets base stats; `RacialTraits` picks racial abilities. Before finalize, `applyAllEffects` sorts the dependencies ahead of each phase's auras: an AP-to-spell-power dependency must see the AP multipliers, as the server's does |
...
- Item effects: `sim/common/{wotlk,tbc}`. Class set bonuses and item effects: `sim/<class>/items.go`,
  registered through `core.ClassEffect`, which gives another class's wearer none (the server's bonus only
  touches its class's spells); `sim/item_wearers_test.go` sims each on every class that can wear it. An effect's closure
