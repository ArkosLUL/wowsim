// Command simval replays mod-sim-validation's recordings through the sim's combat
// tables and reports where the two disagree.
//
// The module writes one JSON line per `.simval` command to the worldserver's
// <LogsDir>/simval/simval.jsonl, with a snapshot of both units and the table the
// server derived from them. This rebuilds that table from the snapshot and
// compares, so a mismatch points at the sim's formula rather than at noise.
//
...
const fixtureDir = "sim/core/testdata/simval"

func main() {
	records := flag.String("records", "/ac/env/dist/logs/simval/simval.jsonl", "the module's JSONL recording")
	fixture := flag.Bool("fixture", false, "also copy the records into "+fixtureDir)
	verbose := flag.Bool("v", false, "print passing checks too")
	flag.Parse()
